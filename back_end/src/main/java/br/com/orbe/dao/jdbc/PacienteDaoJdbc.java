...
public class PacienteDaoJdbc extends AbstractJdbcDao {
    ...
    public List<PacienteResumo> listar() {
        String sql = """
                SELECT CONCAT('U:',u.usuario_id) id, 'TITULAR' tipo, u.nome, u.cpf,
                       u.data_nascimento, u.telefone, u.status, NULL responsavel_id
                ...
                SELECT CONCAT('D:',d.dependente_id), 'DEPENDENTE', d.nome, d.cpf,
                       d.data_nascimento, NULL, d.status, MIN(ud.usuario_id)
                  FROM dependente d
        ...
        try (var connection=connectionFactory.open(); var statement=connection.prepareStatement(sql); var rs=statement.executeQuery()) {
            while(rs.next()) {
                ...
                item.setTelefone(rs.getString("telefone")); item.setStatus(rs.getString("status")); item.setResponsavelId(nullableLong(rs,"responsavel_id")); itens.add(item);
            }
            return itens;
    ...
    public void atualizarDependente(Long id, SalvarPacienteRequest request) {
        try(var connection=connectionFactory.open(); var statement=connection.prepareStatement("UPDATE dependente SET nome=?,cpf=?,data_nascimento=?,status=? WHERE dependente_id=?")) {
            statement.setString(1,request.getNome()); statement.setString(2,request.getCpf()); statement.setObject(3,request.getDataNascimento()); statement.setString(4,request.getStatus()); statement.setLong(5,id); statement.executeUpdate();
        } catch(SQLException exception){throw persistenceException(exception);}
    }
}
