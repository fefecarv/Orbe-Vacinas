...
public class UsuarioDaoJdbc extends AbstractJdbcDao implements UsuarioDao {
    ...
    private static final String COLUNAS = "usuario_id,nome,cpf,email,senha_hash,telefone,data_nascimento,cep,logradouro,numero,complemento,bairro,cidade,estado,status,verificacao_duas_etapas,ultimo_acesso_em,ultimo_ip,token_recuperacao_hash,token_recuperacao_expira_em,criado_em,atualizado_em";

    public UsuarioDaoJdbc(ConnectionFactory connectionFactory) { super(connectionFactory); }

    @Override
    public Usuario salvar(Usuario u) {
        String sql = "INSERT INTO usuario (nome,cpf,email,senha_hash,telefone,data_nascimento,cep,logradouro,numero,complemento,bairro,cidade,estado,status,verificacao_duas_etapas) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public Usuario atualizar(Usuario u) {
        String sql = "UPDATE usuario SET nome=?,cpf=?,email=?,senha_hash=?,telefone=?,data_nascimento=?,cep=?,logradouro=?,numero=?,complemento=?,bairro=?,cidade=?,estado=?,status=?,verificacao_duas_etapas=? WHERE usuario_id=?";
        try (var c = connectionFactory.open(); var ps = c.prepareStatement(sql)) {
            preencher(ps, u); ps.setLong(16, u.getId()); ps.executeUpdate(); return u;
        } catch (SQLException e) { throw persistenceException(e); }
    }
    ...
    private void preencher(java.sql.PreparedStatement ps, Usuario u) throws SQLException {
        ...
        ps.setString(13,u.getEstado()); ps.setString(14,u.getStatus().name()); ps.setBoolean(15,u.isVerificacaoDuasEtapas());
    }
    ...
    private Usuario mapear(ResultSet rs) throws SQLException {
        ...
        u.setEstado(rs.getString("estado")); u.setStatus(StatusUsuario.valueOf(rs.getString("status"))); u.setVerificacaoDuasEtapas(rs.getBoolean("verificacao_duas_etapas"));
