...
public interface AdministracaoService {
    List<UsuarioAdministrativoItem> listarUsuarios(); List<Vacina> listarVacinas(); List<Lote> listarLotes(); Lote salvarLote(Lote lote);
    ...
    List<ConfiguracaoAgenda> listarHorarios(); ConfiguracaoAgenda salvarHorario(ConfiguracaoAgenda item);
    List<BloqueioAgenda> listarBloqueios(); BloqueioAgenda salvarBloqueio(BloqueioAgenda item);
    Auditoria registrarAuditoria(Auditoria item);
}
